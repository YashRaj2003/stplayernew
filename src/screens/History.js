...
export default function History() {
    ...
    async function getWatchHistory() {
        try {
            let repsonse = await axios.get(`http://192.168.1.4:8080/videos/watchHistory/${userinfo?._id}`);
        ...
        <View style={{ paddingBottom: 10, backgroundColor: "#0d0d0d", height: "100%", }} >
            <View style={{ flex: 1, height: "100%", }}>
                ...
                <View style={{ paddingHorizontal: 25, paddingVertical: 10, borderBottomColor: "#202020", borderBottomWidth: 1, backgroundColor: "#161616" }}>
                    <Text style={{ fontFamily: "Poppins_500Medium", letterSpacing: 1.2, color: "#8a8a8a", fontSize: 16, textAlign: "center" }}>WATCH HISTORY</Text>
                </View>
                {videos.length > 0 ?
                    ...
                    <View style={{ flex: 1, height: "100%", padding: 20, display: "flex", alignItems: "center", justifyContent: "center" }}>
                        <Text style={{ color: "white", letterSpacing: 0.4 }}>There is no watch history available for your account</Text>
                    </View>
                }
            </View>
        </View>
